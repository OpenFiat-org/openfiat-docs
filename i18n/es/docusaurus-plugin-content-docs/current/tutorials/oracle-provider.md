---
title: Registra un proveedor de oráculo
---

# Registra un proveedor de oráculo

Regístrate en el Registro de Servicios (OFS-1500) de un nodo como Proveedor de
Oráculo, y luego publica un registro de tipo de cambio firmado (OFS-7000). El
tipo mediano entre todos los proveedores de un par es lo que referencian los
...
Fuente completa: [`examples/oracle_provider.rs`](https://github.com/OpenFiat-org/openfiat-sdks/blob/main/rust/examples/oracle_provider.rs)
(`cargo run --example oracle_provider` desde `openfiat-sdks/rust`).
...
Fuente completa: [`examples/oracle_provider.ts`](https://github.com/OpenFiat-org/openfiat-sdks/blob/main/typescript/examples/oracle_provider.ts)
(`pnpm tsx examples/oracle_provider.ts` desde `openfiat-sdks/typescript`).
